...
export default function PC120Map() {
  ...
  return <main className="viewer-shell">
    ...
    <aside className={`control-panel ${sidebarOpen ? 'is-open' : ''}`}>
      ...
      <section className="control-section"><label className="control-label">Context layers</label><div className="layer-list">{overlays.map((item) => { const unavailable = mode === 'operative' && item.id !== 'withdrawal'; return <label className={`layer-row ${unavailable ? 'is-disabled' : ''}`} key={item.id}><span className="layer-swatch" style={{ backgroundColor: item.id === 'height' ? tenStoreyColour : item.colour }} /><span>{item.label}</span><Switch disabled={unavailable} checked={Boolean(activeOverlays[item.id])} onCheckedChange={(checked) => toggleOverlay(item.id, checked)} /></label>; })}</div>{activeOverlays.height && <div className="storey-controls"><div className="storey-key"><span style={{ backgroundColor: '#eb7d17' }} /><b>6 storeys</b><small>22m / 22m*</small></div><div className="storey-key"><span style={{ backgroundColor: '#d94b16' }} /><b>8 storeys</b><small>27m / 27m*</small></div><label className="storey-key editable"><input type="color" value={tenStoreyColour} onChange={(event) => setTenStoreyColour(event.target.value)} aria-label="10-storey colour" /><b>10 storeys</b><small>34.5m / 34.5m*</small></label><label className="storey-key editable"><input type="color" value={fifteenStoreyColour} onChange={(event) => setFifteenStoreyColour(event.target.value)} aria-label="15-storey colour" /><b>15 storeys</b><small>50m / 50m*</small></label><label className="layer-row estimate-colour-toggle"><span className="layer-swatch estimate-swatch" /><span>Height variation control storey estimate colours</span><Switch checked={showEstimatedStoreys} onCheckedChange={(checked) => commitView({ showEstimatedStoreys: checked })} /></label>{showEstimatedStoreys && <p className="estimate-note">Indicative estimates: 2 storeys at 7–9m, 3 at 13m, 4 at 15–16m, 5 at 18–19.5m, 6 at 21–22.5m, 7 at 24–25m, 9 at 30–32.5m, 15 at 48.5m, and 23 at 75m.</p>}<label className="layer-row business-colour-toggle"><span className="layer-swatch business-swatch" /><span>Colour Business zones</span><Switch checked={colourBusinessZones} onCheckedChange={(checked) => commitView({ colourBusinessZones: checked })} /></label></div>}</section>
